...
// helper: the set of valid member type IDs used across tests.
...
// helper: a fully valid registration we can tweak per test.
...
func TestValidate_RequiredFields(t *testing.T) {
	r := Registration{} // everything empty
	errs := r.Validate(validIDs())
...
func TestValidate_Phone(t *testing.T) {
	cases := []struct {
		...
		{"+4787654322", false},  // valid
		{"+1234567", true},      // 7 digits, too few
		{"87654322", true},      // no country code
		{"+47876543221234567", true}, // too many digits
		{"", true},              // empty
	}
	for _, c := range cases {
